echo ""
        inputtarget(){
        clear
        figlet -w 100 -f small Mass Recon
        ...
        waf
        domain_enum(){
        mkdir -p $domain $domain/sources $domain/result $domain/result/nuclei $domain/result/wayback $domain/result/httpx $domain/result/exploit $domain/result/js $domain/result/gf
