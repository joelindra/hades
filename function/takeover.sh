...
# Takeover check
check_takeover() {
    echo -e "\n${BLUE}[+] Initiating subdomain takeover vulnerability scan...${NC}"
    ...
    if [[ -f "$workspace/result/httpx/httpx.txt" ]]; then
        ...
        for service in "${high_risk_services[@]}"; do
            ...
            count=$(grep -i "$service_domain" "$workspace/result/takeover/consolidated_results.txt" | wc -l)
            
            if [ $count -gt 0 ]; then
                found_high_risk=true
                echo -e "${RED}[!] Found $count potential $service_name ($service_domain) takeover vulnerabilities!${NC}"
                echo -e "${MAGENTA}[*] Vulnerable subdomains:${NC}"
                grep -i "$service_domain" "$workspace/result/takeover/consolidated_results.txt" | while read -r line; do
                    echo -e "${CYAN}    └── $line${NC}"
                done
            fi
        done
...
<html>
<head>
    <title>Subdomain Takeover Vulnerability Report</title>
    <style>
        body { 
...
EOF
        ...
        for service in "${high_risk_services[@]}"; do
            service_domain="${service%%:*}"
            service_name="${service#*:}"
            if grep -qi "$service_domain" "$workspace/result/takeover/consolidated_results.txt"; then
                cat << EOF >> "$workspace/result/takeover/report.html"
            <div class="service-section">
                <div class="service-name">$service_name ($service_domain)</div>
                $(grep -i "$service_domain" "$workspace/result/takeover/consolidated_results.txt" | while read -r line; do
        ...
        cat << EOF >> "$workspace/result/takeover/report.html"
            <div class="service-section">
                ...
                $(cat "$workspace/result/takeover/consolidated_results.txt" | while read -r line; do
                    ...
                    for service in "${high_risk_services[@]}"; do
                        service_domain="${service%%:*}"
                        if echo "$line" | grep -qi "$service_domain"; then
                            is_high_risk=true
                            break
                        fi
                    done
...
generate_report() {
    ...
    {
        echo "Subdomain Takeover Scan Report"
        echo "=============================="
        echo "Date: $(date)"
...
send_to_telegram() {
    if [[ -f telegram_token.txt && -f telegram_chat_id.txt ]]; then
        ...
        message="🔍 Subdomain Takeover scan completed for: $domain
📊 Summary:
...
main() {
    ...
    generate_report
    send_to_telegram
    echo -e "\n${GREEN}[✓] Subdomain Takeover scan completed successfully!${NC}\n"
}
