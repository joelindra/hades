...
display_banner() {
    clear
    echo -e "${BLUE}"
    figlet -w 100 -f small "XSS Vulnerability Scanner"
    echo -e "${NC}"
    echo -e "${MAGENTA}[*] Advanced Cross-Site Scripting Detection Tool${NC}"
...
collect_wayback() {
    ...
    mkdir -p $domain/{sources,result/{gf,wayback,xss}}
...
send_to_telegram() {
    ...
    message="🔍 XSS scan completed for domain: $domain\n📤 Sending results..."
