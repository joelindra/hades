# Empty line and clear the terminal
echo ""
clear

# Display a banner
figlet -w 100 -f small Single Domain Recon
echo -e $MAGENTA
...
folder() {
    mkdir -p $domain $domain/sources $domain/result $domain/result/wayback $domain/result/gf
}
folder
...
# Wayback machine URL retrieval and filtering
wayback() {
    echo "https://$domain/" | waybackurls | anew $domain/result/wayback/wayback-tmp.txt
    cat $domain/result/wayback/wayback-tmp.txt | egrep -v "\.woff|\.ttf|\.svg|\.eot|\.png|\.jpeg|\.jpg|\.png|\.css|\.ico" | sed 's/:80//g;s/:443//g' | sort -u > $domain/result/wayback/wayback.txt
    rm $domain/result/wayback/wayback-tmp.txt
}
wayback

# Validate URLs using ffuf
valid_url() {
...
# GF pattern search for various vulnerabilities
gf_patt() {
