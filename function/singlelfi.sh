echo ""
        inputtarget(){
        clear
        figlet -w 100 -f small Local File Inclusion
        echo -e $MAGENTA
        ...
        waf
        wayback(){
        mkdir -p $domain $domain/sources $domain/result $domain/result/gf $domain/result/wayback $domain/result/lfi
        ...
        gf_patt
        lfi(){
        cat "$domain/result/gf/lfi.txt" | \
        xargs -I% -P 25 sh -c 'if curl -s "%" 2>/dev/null | grep -q "root:x"; then echo -e "\033[0;31mVulnerable! %\033[0m"; else echo "Not vulnerable: %"; fi' >> "$domain/result/lfi/lfi.txt"
        }
        lfi
        send_to_telegram(){
